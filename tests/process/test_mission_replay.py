...
def test_mission_replay_runs_cross_platform_without_execution_or_state(tmp_path: Path):
    ...
    environment = {**os.environ, "MISSION_REPLAY_PORT": str(port), "MISSION_REPLAY_TOKEN": token}
    for name in tuple(environment):
        if name.startswith("GOOGLE_") or name in {"GEMINI_API_KEY", "GOOGLE_API_KEY", "GRAPHENE_LINEAGE_DB"}:
...
"""
    process = subprocess.Popen([sys.executable, "-c", code], cwd=tmp_path, env=environment, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    connection = http.client.HTTPConnection("127.0.0.1", port, timeout=3)
    try:
        deadline = time.monotonic() + 5
        while time.monotonic() < deadline:
            try:
                connection.request("GET", "/api/mission-control/health", headers={"Authorization": f"Bearer {token}"})
                response = connection.getresponse()
                if response.status == 200:
                    break
            except OSError:
                time.sleep(.05); connection.close(); connection = http.client.HTTPConnection("127.0.0.1", port, timeout=3)
        else:
        ...
        connection.request("GET", f"/api/mission-control/missions/{replay.mission_id}/snapshot", headers={"Authorization": f"Bearer {token}"})
        ...
        page = connection.getresponse(); assert page.status == 200; assert b"Mission Control" in page.read()
    finally:
        connection.close(); process.terminate(); stdout, stderr = process.communicate(timeout=5)
