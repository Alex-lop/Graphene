...
def test_every_relative_markdown_link_resolves() -> None:
    ...
    for document in ROOT.rglob("*.md"):
        if any(part.lower() in {".git", ".venv", "all_md_files"} for part in document.parts):
            continue
...
def test_canonical_docs_have_current_claim_discipline() -> None:
    ...
    assert "does not claim approved context caused or improved" in lowered
    assert "graph-derived context is not fed back" in lowered
...
def test_product_contract_is_canonical_and_legacy_contract_is_honest() -> None:
    ...
    assert "golden_path.json" in history and "compatibility-only" in history
