...
from graphene.viewer.projection import _add_edge, _build_graph, snapshot_at_cursor
...
def test_checked_in_replay_is_a_canonical_sanitized_view_contract():
    ...
    stages = [replay["snapshot"], *(item["snapshot"] for item in replay["deltas"])]

    snapshots = [GraphSnapshot.model_validate(item) for item in stages]
    ...
    assert all(
        item["type"] == "reset" and item["cursor"] == item["snapshot"]["cursor"]
        for item in replay["deltas"]
    )
    ...
    assert replay["meta"]["decision_proof"] == "SIMULATED OPERATOR — NOT HUMAN ATTESTATION"
...
def test_shared_evidence_nodes_are_order_independent_and_final_cursor_is_exact(family):
    ...
    assert evidence.metadata == {"shared_reference": True}
...
def test_projection_uses_bounded_public_rail_and_keeps_latest_nodes():
    ...
    nodes, edges, _heads, omitted = _build_graph({"run_over_cap": tuple(events)})
...
def test_committed_event_streams_once_within_visibility_target(family):
    ...
    try:
        ...
        assert first["type"] == "reset" and "delta" not in first
        assert first["current_id"] == current_node_id(build_snapshot(path, "run_source"))
        assert first["cursor"] == first["snapshot"]["cursor"]
        ...
        assert sum(
            node["id"] == f"event:run_source:{first_event.event_id}"
        ...
        assert time.monotonic() - started <= 0.5
        assert second["type"] == "reset" and second["cursor"] != first["cursor"]
        assert second["current_id"] == current_node_id(build_snapshot(path, "run_source"))
        assert sum(
            node["id"] == f"event:run_source:{event_id}"
            for node in second["snapshot"]["nodes"]
