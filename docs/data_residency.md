...
| `promotion_receipt` | Candidate patch/tree, legacy reconstructed-base field, approval, and authoritative fixed-test bindings; it is not the final local Git result |
...
| `local_commit_receipt` | Explicit isolated local commit, parent/tree, candidate, changed-path, bound-test, and approval bindings; no push, PR, or deployment |
