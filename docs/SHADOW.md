...
Status: v0 design accepted 2026-08-22; the v0 implementation (`backend/graphene/shadow/`, `graphene shadow`) landed the same day. Credential-free tests pass for the ndjson path on the synthetic fixture under `tests/fixtures/shadow/ndjson/` and, since 2026-08-23, for the `claude-code` adapter on the synthetic fixture under `tests/fixtures/shadow/claude-code/`. The `claude-code` adapter was built against one real Claude Code 2.1.x transcript kept privately under `local/shadow/` (gitignored); that transcript ingested with zero unknown records and the report and lint ran, and only the counts are recorded in [`contracts/product_proof.json`](../contracts/product_proof.json). Because that evidence is not reproducible from the repository, the real-session report stays **NOT PROVEN**; the README proof table and the contract flip a label only in the same change as the machine evidence that earns it.
...
The capsule is a new `0700` directory holding `manifest.json`, `events.ndjson`, `graph.json`, `lint.json`, `report.txt`, and `VERIFY.md`; the manifest pins every other file by SHA-256 and size. Re-ingesting a capsule: `graphene shadow ingest CAPSULE/events.ndjson --format ndjson` reproduces the same events, event ids, and session digest, because the exported stream already contains its inferred claim events so no matcher runs and no renumbering occurs; the `SHADOW_ID` is the same for an ndjson-origin session and differs for a claude-code-origin one, because the identifier binds the ingesting adapter. And `python -m graphene.shadow.verify CAPSULE` recomputes every `event_id`, the session digest, every file digest, the `shadow_id`, and the `segments.v1`/`lint.v1` outputs from the capsule's bytes alone.

Two adapters, exactly:

- `claude-code` parses a local Claude Code session transcript (the JSONL file under `~/.claude/projects/<project>/<session>.jsonl`), versioned `1.0.0` against the record shapes observed in one real Claude Code 2.1.x session. Records are taken in file order (the `parentUuid` chain order of the observed transcript; a parent that names a later record fails closed). `user` and `assistant` text becomes `message` events (an `isMeta` user record is the `system` actor; `thinking` blocks are hidden reasoning and are never ingested); each `tool_use` block maps by tool name: `Bash` through `classify.v1` into `command_exec`, `check_run`, `vcs_op`, `network_op`, or `install_op` plus inferred `file_*` events for `rm`/`mv`/`cp`/`touch`/`sed -i`/`tee`/redirections, `Read` to `file_read`, `Edit`/`MultiEdit`/`NotebookEdit` to `file_edit`, `Write` to `file_create`, `WebFetch`/`WebSearch` to `network_op`, and every other tool to `tool_call`; each `tool_result` block becomes the matching `check_result`, `command_result`, or `tool_result` (`exit_code` 0 when `is_error` is false, the `Exit code N` line when it is true). Paths are classified against `--repo`, else the record's `cwd`; writes keep only a SHA-256 of the written text; excerpts are bounded, secret-scrubbed, and have the home directory collapsed to `~`. Bookkeeping records (`attachment`, `file-history-*`, `queue-operation`, `mode`, `permission-mode`, `ai-title`, `last-prompt`, `atis-latch`, `summary`, `system` `turn_duration`) carry no agent action and are counted by type in the session summary (`skipped_records`), never silently dropped; any record or block the adapter cannot classify, and a `tool_result` with no earlier `tool_use`, becomes an explicit `unknown` event the report counts. A malformed line, a missing or mistyped required field, a bad timestamp, an unusable tool path, an over-long line, or over-deep nesting fails closed with an error naming the line and the field, and nothing is persisted. Not yet surfaced: `edited_text_file` attachments (external edits), sidechain and resumed-session structure beyond accepting a parent outside the file, and tool names longer than 64 characters, which are rejected.
