...
`graphene mission demo` selects the live `gemini-adk` Taskmaster path, defaults to two workers, and persists a model-proposed plan for explicit approval. The gated process smoke now covers the full two-worker mission, distinct provider/session/invocation/workspace/lease identities, measured overlap, accepted-only assembly, verification, and source-checkout sovereignty. The 2026-08-23 live run and 3/3 rehearsal on 2026-08-24 are historical earlier-runtime evidence; the current recovery runtime is **NOT PROVEN** live (see [Proof](PROOF.md)). Do not present implementation or fake-model tests as live Gemini proof — the historical claim rests on receipts in `evidence/`, not on them. The Docker check executor has one captured CI smoke — a scoped fixture check inside the built image on `ubuntu-24.04` ([evidence](../evidence/docker/2026-09-07/README.md)) — and no live mission has run through it. The official Firestore emulator production path is separately **VERIFIED_LOCAL (4 passed)**. Real cloud proof requires the [owner checklist](ALEX_CLOUD_SETUP.md).
