...
The cloud slice contains typed one-worker registration/claim/fetch/heartbeat/completion contracts, transactional Firestore outbox transitions, a separately packaged private multi-mission coordinator, a bounded HTTPS client that obtains a fresh audience token, and an outbound local-executor loop. An operator seed command binds a still-proposed local schema-2 plan to a separate schema-1 Firestore execution mission and receipt; it is not a continuous authority projection. The official emulator verifies that seed plus production create/approve/readiness/claim/heartbeat/completion and sharded materialization/reconcile. Abandon is disabled, so interrupted claims wait for lease TTL expiry. The adapter is not yet the complete `SchedulerStore`: worker registration/revocation, lease expiry/recovery, generic `claim_task`/`heartbeat`/`complete_attempt`, `enter_awaiting_result`, pause, resume, and cancel remain unsupported there. No service was deployed and live cloud proof remains open.
