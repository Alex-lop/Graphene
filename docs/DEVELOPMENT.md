...
`GRAPHENE_CHECK_EXECUTOR` selects how `fixture-tests` checks run: `docker` or `host-sandbox` (macOS `sandbox-exec`, check subprocess registered in the owned-process registry). The `gemini-adk` path defaults to `docker`. `scripted-local` binds its executor per mission at acceptance — an unset variable binds the host sandbox, the executor its proof row was captured under — and that binding then wins over any later change to the variable; only a mission that carries no binding reads the environment. A Docker check registers no owned process, so `mission cancel` cannot reap its container. Any other value fails closed with `GRAPHENE_CHECK_EXECUTOR must be docker or host-sandbox`. The darwin-gated tests in `tests/unit/orchestration/test_host_check_runner.py` drive the host runner through a fake two-worker mission; every worker attempt also binds a `worker-provider-receipt` evidence artifact that `store.verify` resolves by digest.

`graphene mission demo` is the credential-gated live Taskmaster planner/worker entrypoint; it has no fake or replay fallback. Its 2026-08-23 two-worker provider receipts are historical earlier-runtime evidence; the current recovery runtime is **NOT PROVEN** live (see [Proof](PROOF.md)). Live Firestore/Cloud commands require the exact project checks in [Alex cloud setup](ALEX_CLOUD_SETUP.md). Real Cloud Run/Firestore, benchmark results, and the submission video remain **NOT PROVEN**; the Docker check executor is `VERIFIED_CI` for one scoped fixture smoke only (see [Proof](PROOF.md)). Do not convert required deterministic tests into skips.
