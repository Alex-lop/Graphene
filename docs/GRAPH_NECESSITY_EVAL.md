...
5. Timing does not prove causality; only explicit committed fixture evidence is shown; activity is not importance, impact, or correctness. Authorized source/diff/test-output bytes may exist only in private live-driver artifacts and are not publicly projected. Live agent behavior, new test execution, arbitrary shell/editor work, deployment, and external model behavior are not observed by this replay.
