...
| Result inspection/export | `mission result show/export` verifies the V2-bound candidate and writes a create-only private patch | Final immutable-tree release matrix |
| Replay final decision | Pending-candidate checkpoint and simulated continuation exist; replay is not V2 bundle proof | Final generator/process check against the release fingerprint |
...
| Repository mutations | Generic worker intent writes text files only; delete/rename/mode/symlink/submodule are not end-to-end supported, and scripted deletion staging is unproven | Explicit trusted mutation operations plus runtime/assembly/final-tree tests |
...
| Resource Sentinel | Sampled-pressure controller is unit tested but not wired into the product Gemini scheduler | Instantiate it with trusted process sampling and capture pause/reduce/resume evidence |
